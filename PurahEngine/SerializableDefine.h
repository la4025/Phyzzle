...
#define POSTDESERIALIZE_VECTOR(value)\
{\
	for(int i = 0; i < jsonData[#value].size() ; i++)\
	{\
		value.push_back(static_cast<decltype(value)::value_type>(PurahEngine::FileManager::GetInstance().GetAddress(jsonData[#value][i]))); \
	}\
}