...
PS_OUT main(PixelIn input)
{
    ...
    CalculateLight(lightIndex, viewNormal, viewPos, input.uv, color.diffuse, color.ambient, color.specular, float3(0, 0, 0), float4(0, 0, 0, 0));
    
    output.diffuse = float4(saturate(color.diffuse + color.ambient).xyz, 1.0f);
