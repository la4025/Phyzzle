...
namespace ZonaiPhysics
{
		...
		PxVec3p& cA2wOut, PxVec3p& cB2wOut)
	{
		...
		if (linearDistance > data.linearLimit)
		{
			...
			c->flags = Px1DConstraintFlag::eRESTITUTION;              // 스프링 플래그 설정
			++c;
		}
		else
		{
			// 위치가 제한을 넘지 않은 경우
			linearError = data.linearLimit - linearDistance;          // 제한을 넘지 않은 거리 계산
			linearDirection.normalize();    // 방향 벡터 정규화
		...
		c->minImpulse = -PX_MAX_F32;
		c->maxImpulse = PX_MAX_F32;
		c->flags = Px1DConstraintFlag::eSPRING | Px1DConstraintFlag::eANGULAR_CONSTRAINT;                    // 스프링 플래그 설정
	...
	void D1SpringFlexJoint::onOriginShift(const PxVec3& shift)
	{
		...
		// 
		// Adjust the local poses by the shift
		mLocalPose[0].p -= shift;
		mLocalPose[1].p -= shift;

		// Update the constraint to reflect the origin shift
